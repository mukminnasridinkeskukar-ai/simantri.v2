...
- **Bagian 1 — Overview**: Dashboard (5 kartu statistik pop-up + grafik bar sebaran per kecamatan), Petunjuk Penggunaan (dua tab: **Buku Petunjuk Pembaca** — flipbook baca-saja tanpa unduh — dan **Ringkasan Cepat** accordion), Peta Sebaran Praktik (Leaflet, pusat Tenggarong −0.4419, 117.0861), Notifikasi Expired SIP/STR (badge merah/kuning/hijau, H-30), **Cek Hasil Verifikasi** (pencarian hasil verval **izin praktik & fasyankes** — data bersumber dari tabel `verval_izin_praktik` & `verval_fasyankes`, dilengkapi **saran nama otomatis** dari data yang sudah ada dan pencarian kode verifikasi; klik hasil untuk detail lengkap). Nomor STR otomatis **disembunyikan dari pengunjung yang belum masuk** (privasi pembaca; penuh hanya untuk pengguna login).
...
- **v1.7.0** — **Pengembangan menu Cek Hasil Verifikasi (Bagian 1)**: sumber data kini **langsung dari tabel verval** — `verval_izin_praktik` & `verval_fasyankes` (tidak lagi dari tabel data tenaga/fasyankes/praktik), sehingga yang tampil adalah **hasil verifikasi yang resmi**. Kolom pencarian dilengkapi **saran nama otomatis (autocomplete)** dari data yang sudah ada: klik kolom pencarian → daftar nama dikelompokkan **Izin Praktik** & **Fasyankes** lengkap dengan badge status (SIP / Layak-Perbaikan-dst.), unit kerja/jenis & kecamatan — bisa difilter sambil mengetik, diklik untuk langsung mencari, dan didukung navigasi keyboard (panah/Enter/Escape). Pencarian juga mencakup **kode verifikasi** (`VP-…`/`VF-…`) serta unit kerja/nomor unit/kecamatan; hasil dikelompokkan per jenis dengan jumlah, dan klik baris membuka **detail lengkap verval** (No. STR tetap termask bagi yang belum login; tombol Edit/Hapus tetap mengikuti aturan Admin/Operator). Tanpa perubahan skema database.
